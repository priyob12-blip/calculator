...
if shape == "Trapesium":
    ...
    # EDIT: Dikurangi menjadi 3 kolom (menghapus Proteksi & Jenis Tank)
...
if st.button("💾 HITUNG SEKARANG", type="primary", use_container_width=True):
    ...
    # EDIT: Menghilangkan rumus f_build (faktor proteksi/atap)
    # Shell to Building: Jarak ke Jalan Umum/Internal (Murni Tabel NFPA)
    tank_to_road = round(max(min_dist_road, 1.5), 2)
    
    # EDIT: Menghilangkan rumus f_prop (faktor proteksi/atap)
    # Shell to Property: Jarak ke Fasilitas Bangunan/Batas Properti (Murni Tabel NFPA)
    tank_to_prop = round(max(min_dist_fac, 1.5), 2)
